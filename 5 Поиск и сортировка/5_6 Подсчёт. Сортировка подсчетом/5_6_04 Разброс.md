...
fn main() {
    ...
    /* Counting sort preparation */
    let range = (max_value - min_value + 1) as usize;
    let mut counting = vec![0usize; range];

    for &num in &numbers {
        counting[(num - min_value) as usize] += 1;
    }

    /* Reconstruct the sorted list */
    let mut sorted = Vec::with_capacity(length);
    for (idx, &cnt) in counting.iter().enumerate() {
        let value = idx as i32 + min_value;
        for _ in 0..cnt {
            sorted.push(value);
        }
    }
