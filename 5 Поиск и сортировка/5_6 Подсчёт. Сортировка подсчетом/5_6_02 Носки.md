...
count = [0]*M # счетчик для результата
for i in range(len(points)):
    for a,b in socks:
        if points[i] in range(a,b+1):count[i]+=1
print(*count,sep = '\n')
...
fn main() {
    ...
    // First line: _l (unused), n, m
    let _l = nums.next().unwrap();                // _l is not used later
    let n = nums.next().unwrap() as usize;        // number of socks (intervals)
    let m = nums.next().unwrap() as usize;        // number of points
    ...
    for _ in 0..m {
        points.push(nums.next().unwrap());
    }

    // Count how many intervals contain each point
    ...
    // Output each count on its own line
    let result = count
        .iter()
        .map(|c| c.to_string())
        .collect::<Vec<_>>()
