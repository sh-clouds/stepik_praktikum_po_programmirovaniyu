...
fn main() {
    ...
    // Split into lines: the first line is ignored, the second line is the string `s`
    let mut lines = input.lines();
    let _ = lines.next(); // discard the first input line
    let s = lines.next().unwrap_or("").trim_end();

    // Count occurrences of each character, keeping keys sorted (BTreeMap)
    let mut freq: BTreeMap<char, usize> = BTreeMap::new();
    for ch in s.chars() {
        *freq.entry(ch).or_insert(0) += 1;
    }

    // Build the left half of the palindrome and find a possible middle character
    let mut left = String::new();
    let mut middle = String::new();

    for (&c, &count) in freq.iter() {
        // Append `c` count/2 times to the left side
        left.extend(std::iter::repeat(c).take(count / 2));

        // If we haven't chosen a middle character yet and the count is odd, use it
        if middle.is_empty() && count % 2 == 1 {
            middle.push(c);
        }
    }
    ...
    // Assemble and output the final palindrome
