...
fn main() {
    // Read the whole stdin content
    let mut input = String::new();
    io::stdin().read_to_string(&mut input).expect("Failed to read stdin");
    let mut lines = input.lines();

    // First line: increase counters
    ...
    // Determine if any count is non‑zero
    let any_non_zero = counter.values().any(|&v| v != 0);

    // Print result: "YES" if all zero (i.e., strings are anagrams), otherwise "NO"
    if any_non_zero {
        println!("NO");
    } else {
        println!("YES");
