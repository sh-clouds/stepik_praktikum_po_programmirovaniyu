...
fn main() {
    ...
    // ----- parse data -----
    // number of elements
    let n: usize = iter.next().unwrap().parse().expect("expected n");

    // array c : n integers
    let mut c: Vec<i64> = Vec::with_capacity(n);
    for _ in 0..n {
        let val: i64 = iter.next().unwrap().parse().expect("expected integer for c");
        c.push(val);
    }

    // number of elements in comb (unused later, but required by input format)
    let k: usize = iter.next().unwrap().parse().expect("expected k");

    // array comb : k integers
    let mut comb: Vec<usize> = Vec::with_capacity(k);
    for _ in 0..k {
        let val: usize = iter.next().unwrap().parse().expect("expected integer for comb");
        comb.push(val);
    }

    // ----- compute occurrence counts (nk) -----
    // nk[i] stores how many times (i+1) appears in comb
    let mut occurrence_counts = vec![0usize; n];
    for &value in &comb {
        if (1..=n).contains(&value) {
            occurrence_counts[value - 1] += 1;
        }
    }
    ...
    for i in 0..n {
        if c[i] < occurrence_counts[i] as i64 {
            output.push_str("yes\n");
        } else {
            output.push_str("no\n");
        }
