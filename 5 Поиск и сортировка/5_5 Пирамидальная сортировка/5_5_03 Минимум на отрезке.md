...
fn main() {
    ...
    // The remaining numbers form the list `nums`
    let nums: Vec<i64> = numbers.take(n).collect();

    // For each sliding window of size `k`, print its minimum value
    for i in 0..=n - k {
        let window_min = nums[i..i + k].iter().min().unwrap();
        println!("{}", window_min);
    }
}
