Линейный поиск - 3

Напишите программу, которая выводит номера элементов массива, равных данному числу.

Входные данные
В первой строке задается одно натуральное число N, не превосходящее 1000 – размер массива.
Во второй строке вводятся N чисел – элементы массива (целые числа, не превосходящие по модулю 1000).
В третьей строке содержится одно целое число x, не превосходящее по модулю 1000.

Выходные данные
Вывести через пробел номера элементов, равных данному, в порядке возрастания. Если таких элементов нет, ничего выводить не нужно.

Sample Input:
5
1 2 3 4 5
3

Sample Output:
3

Напишите программу. Тестируется через stdin → stdout
Верно решили 453 учащихся
Из всех попыток 51% верных


# ruby
n = gets.to_i
a = gets.chomp.split(' ').map { |j| j.to_i }
x = gets.to_i

1.upto(n) do |i|
    if a[i] == x
        print "#{i+1} "
    end
end  



# python
_ = int(input())
lst = list(map(int, input().split()))
n = int(input())
res = [e for e, i in enumerate(lst, 1) if i == n]
print(*res)


# JS
var stdin = process.openStdin();

stdin.addListener("data", function(d) {
    let str = d.toString().split('\n');
    let n = str[2];
    str = str[1].split(' ');
    let result = [];
    for (let i in str) 
        if (n === str[i]) result.push(+i+1); 
    console.log(result.join(' '));
  });


# C#
using System;

public class MainClass
{
    public static void Main()
    {
        int n=int.Parse(Console.ReadLine());
        string[] arr=Console. ReadLine().Split();
        string s=Console.ReadLine();
        bool isFirst=true;
        for(int i=0;i<n;i++)
        {
            if(arr[i]==s)
            {
               if(isFirst)
               {
               Console.Write(i+1);
               isFirst=false; 
               } 
               else
               {
               Console.Write($" {i+1}");
               }                              
            }            
        }
    }
}


# java
import java.util.Arrays;
import java.util.Scanner;
import java.util.stream.IntStream;

public class Main {
    public static void main(String[] args) {
        Scanner input = new Scanner(System.in);

        int length = Integer.parseInt(input.nextLine().trim());
        int[] array = Arrays.stream(input.nextLine().split("\\s+")).mapToInt(Integer::parseInt).toArray();
        int number = Integer.parseInt(input.nextLine().trim());

        IntStream.iterate(0, x -> x + 1)
                .limit(length)
                .filter(x -> array[x] == number)
                .map(x -> x + 1)
                .forEach(x -> System.out.print(x + " "));
    }
}


# C
#include <inttypes.h>
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>

void input(size_t* N, int64_t** arr, int64_t* X);
void echo_array(int64_t** arr, size_t N);
size_t count_x(int64_t** arr, size_t N, int64_t X);
int64_t nearest_x(int64_t** arr, size_t N, int64_t X);
void echo_indexes_x(int64_t** arr, size_t N, int64_t X);

int main() {
    size_t N = 0;
    int64_t* array = NULL;
    int64_t X = 0;
    input(&N, &array, &X);
    if (N) {
        echo_indexes_x(&array, N, X);
        free(array);
    }
    return 0;
}

void echo_indexes_x(int64_t** arr, size_t N, int64_t X) {
    for (size_t i = 0; i < N; i++)
        if (X == (*arr)[i]) printf("%zu%s", i + 1, " ");
}
    
int64_t nearest_x(int64_t** arr, size_t N, int64_t X) {
    int64_t item, delta = LLONG_MAX;
    for (size_t i = 0; i < N; i++)
        if (abs(X - (*arr)[i]) < delta) {
            item = (*arr)[i];
            delta = abs(X - (*arr)[i]);
        }
    return item;
}

size_t count_x(int64_t** arr, size_t N, int64_t X) {
    size_t count = 0;
    for (size_t i = 0; i < N; i++)
        if (X == (*arr)[i]) count++;
    return count;
}

void echo_array(int64_t** arr, size_t N) {
    for (size_t i = 0; i < N; i++)
        printf("%"PRId64"%s", (*arr)[i], i + 1 == N ? "" : " ");
}

void input(size_t* N, int64_t** arr, int64_t* X) {
    int64_t innum;
    *N = (scanf("%"SCNd64, &innum) == 1 && innum > 0) ? innum : 0;
    if (*N > 0) *arr = malloc(*N * sizeof(int64_t*));
    if (*arr == NULL) *N = 0;
    if (*N > 0)
        for (size_t i = 0; i < *N; i++)
            scanf("%"SCNd64, &((*arr)[i]));
    if (*N > 0) scanf("%"SCNd64, &(*X));
}


# C++
#include <iostream>

using namespace std;

int main()
{
    int N, x;
    cin >> N;
    int array[N];
    for(int i = 0; i < N; i++)
        cin >> array[i];
    cin >> x;
    for(int i = 0; i < N; i++)
        if(array[i] == x)
            cout << i + 1 << " ";
    return 0;
}



# rust
use std::io::{self, Read};

fn main() {
    // Read the whole input
    let mut input = String::new();
    io::stdin().read_to_string(&mut input).unwrap();

    // Split into whitespace‑separated tokens
    let mut iter = input.split_whitespace();

    // First value – length of the list (ignored afterwards)
    let len: usize = iter.next().unwrap().parse().unwrap();

    // Read exactly `len` integers into the vector
    let mut lst = Vec::with_capacity(len);
    for _ in 0..len {
        let val: i32 = iter.next().unwrap().parse().unwrap();
        lst.push(val);
    }

    // The target number to search for
    let n: i32 = iter.next().unwrap().parse().unwrap();

    // Collect 1‑based positions where the element equals `n`
    let mut positions = Vec::new();
    for (idx, &value) in lst.iter().enumerate() {
        if value == n {
            positions.push((idx + 1).to_string());
        }
    }

    // Print positions separated by a space (empty line if none)
    println!("{}", positions.join(" "));
}






