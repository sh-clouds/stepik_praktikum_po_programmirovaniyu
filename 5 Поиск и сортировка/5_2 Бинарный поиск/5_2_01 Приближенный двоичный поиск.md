...
fn main() {
    ...
    // Parse the first list `l1`
    let l1: Vec<i64> = lines
        .next()
        .unwrap_or("")
        .split_whitespace()
    ...
    // Parse the second list `l2`
    let l2: Vec<i64> = lines
        .next()
        .unwrap_or("")
        .split_whitespace()
        .map(|s| s.parse().unwrap())
    ...
    // For each element in `l2`, find and print the closest element from `l1`
    for el in l2 {
        let mut closest = l1[0];
        let mut min_diff = (el - closest).abs();

        for &candidate in &l1[1..] {
            let diff = (el - candidate).abs();
            if diff < min_diff {
                min_diff = diff;
                closest = candidate;
            }
