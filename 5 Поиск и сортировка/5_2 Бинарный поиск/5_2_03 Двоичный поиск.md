...
/// Returns `true` if `target` is present in `arr`, otherwise `false`.
fn bin_search(arr: &[i64], target: i64) -> bool {
    let mut left = 0usize;
...
fn main() {
    ...
    // First line: N (size of the array) and K (number of queries).
    let n: usize = it.next().unwrap().parse().unwrap();
    let k: usize = it.next().unwrap().parse().unwrap();

    // Second line: the sorted array `mas`.
    let mut mas: Vec<i64> = (0..n)
        .map(|_| it.next().unwrap().parse().unwrap())
        .collect();

    // Third line: the queries.
    let queries: Vec<i64> = (0..k)
        .map(|_| it.next().unwrap().parse().unwrap())
        .collect();

    // Process each query and output the result.
    for q in queries {
        let answer = if bin_search(&mas, q) { "YES" } else { "NO" };
        println!("{}", answer);
    }
}
