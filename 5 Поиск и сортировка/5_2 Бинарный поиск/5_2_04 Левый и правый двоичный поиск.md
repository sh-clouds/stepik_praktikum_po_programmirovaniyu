...
fn lower_bound(arr: &[i64], target: i64) -> isize {
...
/// If all elements are smaller, returns `arr.len()`.
fn right_bound(arr: &[i64], target: i64) -> isize {
    let mut left: isize = -1;
    let mut right: isize = arr.len() as isize;
    while right - left > 1 {
        let mid = (right + left) / 2;
...
/// Returns `None` when the interval is empty, otherwise returns the
/// 1‑based positions `(l+1, r+1)`.
fn search(arr: &[i64], target: i64) -> Option<(usize, usize)> {
    let l = right_bound(arr, target);
    let r = lower_bound(arr, target);
    if l > r {
        None
    } else {
        Some(((l + 1) as usize, (r + 1) as usize))
    }
}
...
fn main() {
    ...
    // Second line: the sorted array N.
    let n_line = lines.next().unwrap_or("");
    let n: Vec<i64> = n_line
        .split_whitespace()
        .map(|s| s.parse::<i64>().unwrap())
        .collect();

    // Third line: the queries K.
    let k_line = lines.next().unwrap_or("");
    let queries: Vec<i64> = k_line
        .split_whitespace()
        .map(|s| s.parse::<i64>().unwrap())
        .collect();
