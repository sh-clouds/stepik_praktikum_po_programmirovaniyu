...
fn main() {
    ...
    // Split the input into lines and parse each line into a vector of i64 numbers
    let mut lines = input
        .split('\n')
    ...
    // Discard the first line (usually a header) and keep the remaining rows
    let _header = lines.next();
    let mut rows: Vec<Vec<i64>> = lines.collect();

    // Sort rows: primary key – second element descending,
    // secondary key – first element ascending
    rows.sort_by(|a, b| {
        // Assume each row has at least two elements
        let primary = b[1].cmp(&a[1]); // descending by second column
        if primary != std::cmp::Ordering::Equal {
            primary
        } else {
            a[0].cmp(&b[0]) // ascending by first column
        }
    });

    // Output each row, numbers separated by a single space
    for row in rows {
        let line = row.iter()
