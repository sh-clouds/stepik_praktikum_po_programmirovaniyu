...
fn main() {
    ...
    // The first number is n, the length of the list
    let n = numbers.next().expect("missing n") as usize;

    // Collect the next n numbers into a vector
    let mut lst: Vec<i64> = numbers.take(n).collect();

    // Insertion sort
    for i in 0..n.saturating_sub(1) {
        // j goes from i+1 down to 1 (inclusive)
        for j in (1..=i + 1).rev() {
            if lst[j] < lst[j - 1] {
                lst.swap(j, j - 1);
            }
    ...
    // Output the sorted list, space‑separated
    let result = lst
        .iter()
        .map(|v| v.to_string())
        .collect::<Vec<_>>()
