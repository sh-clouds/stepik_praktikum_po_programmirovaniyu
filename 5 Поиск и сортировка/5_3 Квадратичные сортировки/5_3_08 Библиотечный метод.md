Библиотечный метод

Продемонстрируйте работу метода сортировки вставками по возрастанию. Для этого выведите состояние данного массива после каждой вставки на отдельных строках. Если массив упорядочен изначально, то следует не выводить ничего.

Входные данные
На первой строке дано число (1 ≤ N ≤ 100) – количество элементов в массиве. На второй строке задан сам массив: последовательность натуральных чисел, не превышающих 109.

Выходные данные
В выходной файл выведите строки (по количеству вставок) по N чисел каждая.

Sample Input 1:
2
2 1

Sample Output 1:
1 2

Sample Input 2:
4
2 1 5 3

Sample Output 2:
1 2 5 3
1 2 3 5

Напишите программу. Тестируется через stdin → stdout
Верно решили 92 учащихся
Из всех попыток 19% верных


# ruby
n = gets.to_i
a = gets.chomp.split(' ').map{ |j| j.to_i }

1.upto(a.size-1) do |j|
    isSorted = true
    0.upto(n-j-1) do |i|  
        if a[i] > a[i + 1]
            a[i], a[i + 1] = a[i + 1], a[i]
            print a.join(" ")
            print "\n"
            isSorted = false   
        end
    end    
    if (isSorted)
        break
    end        
end


# python
l, m = int(input()), list(map(int, input().split()))
for i in range(l):
    for k in range(l-1-i):
        if m[k] > m[k+1]:
            m[k], m[k+1] = m[k+1], m[k]
            print(*m)


# JS
var stdin = process.openStdin();

stdin.addListener("data", function(d) {
    let str = d.toString().split('\n');
    str = str[1].split(' ');
    for (let i=0; i<str.length-1; i++) 
        for (j=0; j<str.length-i-1; j++)
        if (+str[j] > +str[j+1]) {
            [str[j], str[j+1]] = [str[j+1], str[j]];
            console.log(str.join(' ')); 
    }
  });


  
# C#
using System;

public class MainClass
{
    public static void Main()
    {
       int n=int. Parse(Console.ReadLine());
        int[] arr=Array.ConvertAll(Console.ReadLine(). Split(),int.Parse); 
        bool stop=false;        
        
        while(!stop)
        {
            stop=true;
                                 
        for(var i=0;i<n-1;i++)
        {
           if(arr[i]>arr[i+1])
           {
               (arr[i],arr[i+1])=                               (arr[i+1],arr[i]);
             stop=false;
             for(int x=0;x<n;x++)
        Console.Write(x!=n-1?arr[x] +" ":arr[x]); 
        Console.WriteLine();                 
           }                              
           }                    
        }       
    }
}


# java
import java.util.Arrays;
import java.util.Scanner;

public class Main {
    public static void main(String[] args) {
        Scanner input = new Scanner(System.in);
        input.nextLine();
        int[] array = Arrays.stream(input.nextLine().split("\\s+")).mapToInt(Integer::parseInt).toArray();
        doBubbleSort(array);
    }

    public static int[] doBubbleSort(int[] array) {
        for(int a = 0; a < array.length - 1; a++) {
            for(int b = 0; b < array.length - 1 - a; b++) {
                if(array[b] > array[b + 1]) {
                    int current = array[b];
                    array[b] = array[b + 1];
                    array[b + 1] = current;
                    printTheArray(array);
                }
            }
        }
        return array;
    }

    private static void printTheArray(int[] array) {
        for (int e = 0; e < array.length; e++) {
            if(e < array.length - 1) System.out.print(array[e] + " ");
            else System.out.println(array[e]);
        }
    }
}



# rust
use std::io::{self, Read};

fn main() {
    // Read entire stdin
    let mut input = String::new();
    io::stdin().read_to_string(&mut input).unwrap();

    // Split into whitespace‑separated tokens
    let mut iter = input.split_whitespace();

    // First token: length of the list (not strictly needed, but kept for parity)
    let n: usize = iter
        .next()
        .expect("expected length")
        .parse()
        .expect("length must be an integer");

    // Remaining tokens: the list of integers
    let mut numbers: Vec<i32> = iter
        .take(n)
        .map(|s| s.parse::<i32>().expect("list elements must be integers"))
        .collect();

    // Bubble sort with intermediate printing after each swap
    for i in 0..n {
        // The inner loop runs up to (n - 1 - i)
        for k in 0..(n - 1 - i) {
            if numbers[k] > numbers[k + 1] {
                numbers.swap(k, k + 1);
                // Print the current state of the list
                let line = numbers
                    .iter()
                    .map(|v| v.to_string())
                    .collect::<Vec<_>>()
                    .join(" ");
                println!("{}", line);
            }
        }
    }
}



