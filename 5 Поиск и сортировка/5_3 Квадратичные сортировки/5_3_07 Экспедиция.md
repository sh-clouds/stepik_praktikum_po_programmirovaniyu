...
fn main() {
    ...
    // Split into rows of integers, ignoring completely empty lines
    ...
    // Need at least three rows (first ignored, one ignored before the last, and the last)
    if rows.len() < 3 {
        println!("0");
        return;
    }

    // ----- Separate the data according to an unpacking -----
    // rows[0]      -> ignored
    // rows[1..len-2] -> raft rows (to be flattened)
    // rows[len-2]  -> ignored
    // rows[len-1]  -> mass values
    let raft_rows = &rows[1..rows.len() - 2];
    let mut raft_sizes: Vec<i32> = raft_rows.iter().flat_map(|r| r.iter().cloned()).collect();
    let mut masses: Vec<i32> = rows.last().unwrap().clone();

    // ----- Sort both vectors in ascending order -----
    raft_sizes.sort_unstable();
    masses.sort_unstable();

    // ----- Greedy pairing: largest mass with largest raft that can carry it -----
    let mut matches: usize = 0;
    while let (Some(_), Some(_)) = (raft_sizes.last(), masses.last()) {
        // Take the heaviest remaining mass
        let mass = masses.pop().unwrap();

        // The largest remaining raft size
        let max_raft = *raft_sizes.last().unwrap();

        if mass <= max_raft {
            // This raft can carry the mass
            raft_sizes.pop();
            matches += 1;
        }
        // If the mass is too heavy, we simply discard it (already popped) and continue
    }
