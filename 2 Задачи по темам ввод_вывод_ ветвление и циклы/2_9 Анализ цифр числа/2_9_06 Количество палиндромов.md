...
fn main() {
    // Read the entire input (expected to be a single integer)
    let mut input = String::new();
    io::stdin().read_to_string(&mut input).expect("Failed to read input");
    let limit: u64 = input.trim().parse().expect("Input must be a positive integer");

    // Count numbers whose decimal representation is a palindrome
    let palindrome_count = (1..=limit).filter(|&num| {
        let s = num.to_string();
