...
fn main() {
    ...
    // Split input by whitespace and parse the first two numbers
    let mut numbers = input
        .split_whitespace()
        .filter_map(|s| s.parse::<i32>().ok());

    let first_number = numbers.next().expect("Missing first integer");
    let second_number = numbers.next().expect("Missing second integer");

    // Determine the larger value
    let max_value = if first_number > second_number {
        first_number
    } else {
        second_number
    };
