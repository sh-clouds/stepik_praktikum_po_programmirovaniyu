...
static ARAB: [i32; 13] = [1, 4, 5, 9, 10, 40, 50, 90, 100, 400, 500, 900, 1000];
static ROMAN: [&str; 13] = [
    "I", "IV", "V", "IX", "X", "XL", "L", "XC", "C", "CD", "D", "CM", "M",
];

fn arab_to_roman(mut number: i32) -> String {
    let mut result = String::new();
    let mut i: i32 = (ARAB.len() as i32) - 1; // start from the largest value

    while number > 0 {
        if number >= ARAB[i as usize] {
            result.push_str(ROMAN[i as usize]);
            number -= ARAB[i as usize];
        } else {
...
fn main() {
    // Read the whole input, trim whitespace, and parse it as an integer.
    let mut input = String::new();
    io::stdin()
        .read_to_string(&mut input)
        .expect("Failed to read from stdin");
    let number: i32 = input.trim().parse().expect("Input must be an integer");

    println!("{}", arab_to_roman(number));
}
