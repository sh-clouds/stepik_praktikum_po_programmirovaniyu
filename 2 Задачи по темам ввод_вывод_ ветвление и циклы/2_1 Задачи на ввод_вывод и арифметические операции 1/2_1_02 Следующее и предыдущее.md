...
fn main() {
    ...
    let n: i64 = input.trim().parse().expect("Input must be an integer");

    // Compute the next and previous numbers
    let next = n + 1;
    let prev = n - 1;

    // Output using the same format as the original Python code
    println!(
