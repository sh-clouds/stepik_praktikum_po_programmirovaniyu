Четные числа

Входные данные
Вводятся целые числа a и b. Гарантируется, что a не превосходит b

Выходные данные
Выведите (через пробел) все четные числа от a до b (включительно).

Sample Input:
2
5

Sample Output:
2 4

Напишите программу. Тестируется через stdin → stdout
Верно решили 1 024 учащихся
Из всех попыток 42% верных


# ruby
a,b = gets.to_i, gets.to_i
c = []

a.upto(b) do |i|
    if i%2==0
        c.push(i)
    end
end    
    
puts c.join(' ')   


# python
a = (int(input()) + 1) // 2 * 2
b = 1 + int(input())
print(*range(a, b, 2))


# JS
const readline = require('readline');

const rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout
});

rl.on('line',(a1) => {
    rl.on('line',(b1) => {
        a = parseInt(a1);
        b = parseInt(b1);
         var output = ""; 
        for (a;a<=b;a+=1){
          if (a%2==0){
             
         output += a + " "; }
               
        }
            console.log(output)
    });
});


# C#
using System;

public class MainClass
{
    public static void Main()
    {
        // put your c# code here
        int a,b;
        a = int.Parse(Console.ReadLine());
        b = int.Parse(Console.ReadLine());
        for (int i = a; i <=b; i++)
        {
            if(i % 2 == 0)
            {
                Console.Write($"{i} ");
            }
        }
    }
}

# java
import java.util.Scanner;

class Main {
    public static void main(String[] args) {
        Scanner scan=new Scanner(System.in);
        int a=scan.nextInt();
        int b=scan.nextInt();
        for(int i=a;i<(b+1);i++){
            if (i%2==0){
                System.out.print(i);
                System.out.print(' ');
            }
        }
    }
}


# C
#include <stdio.h>

int main() {
    int a,b,i;
    scanf("%d %d", &a, &b);
    for(i = a; i <= b; i++){
        if (i%2 == 0){
            printf("%d ", i);
        }
    }
    return 0;
}

# C++
#include <iostream>
using namespace std;
int main() {
    int a,b;
    cin >> a >> b;
    for(;a<=b;a++) 
     a%2==0? cout << a:cout << " ";
    return 0;
}


# rust
use std::io::{self, Read};

fn main() {
    // Read all input from stdin
    let mut input = String::new();
    io::stdin().read_to_string(&mut input).unwrap();

    // Parse the two integers a and b
    let mut numbers = input.split_whitespace().map(|s| s.parse::<i64>().unwrap());
    let a = numbers.next().expect("first number missing");
    let b = numbers.next().expect("second number missing");

    // Collect even numbers in the inclusive range [a, b]
    let mut evens = Vec::new();
    for i in a..=b {
        if i % 2 == 0 {
            evens.push(i);
        }
    }

    // Output the result as space‑separated values
    let output = evens
        .iter()
        .map(|n| n.to_string())
        .collect::<Vec<_>>()
        .join(" ");
    println!("{}", output);
}







