...
fn main() {
    ...
    let first_x  = numbers.next().expect("Missing first x");
    let first_y  = numbers.next().expect("Missing first y");
    let second_x = numbers.next().expect("Missing second x");
    let second_y = numbers.next().expect("Missing second y");

    // Determine if they share the same x or y coordinate
    let answer = if first_x == second_x || first_y == second_y {
        "YES"
    } else {
        "NO"
    };
