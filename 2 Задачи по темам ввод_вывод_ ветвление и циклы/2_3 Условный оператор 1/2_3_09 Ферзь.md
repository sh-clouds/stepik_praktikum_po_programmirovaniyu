...
fn main() {
    // Read the entire input (four integers, each possibly on its own line)
    ...
    // Condition: same diagonal OR same row/column
    let same_diagonal = (x1 - x2).abs() == (y1 - y2).abs();
    let same_line = x1 == x2 || y1 == y2;

    let answer = if same_diagonal || same_line { "YES" } else { "NO" };
    println!("{}", answer);
}
