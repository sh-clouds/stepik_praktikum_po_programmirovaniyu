...
fn main() {
    ...
    let mut numbers = input.split_whitespace().map(|s| s.parse::<i64>().expect("Invalid integer"));
    ...
    // Compute the absolute value of the product of differences
    let diff_product = (x1 - x2) * (y1 - y2);
    let answer = if diff_product.abs() == 2 { "YES" } else { "NO" };
