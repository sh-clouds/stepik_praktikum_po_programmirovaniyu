...
fn main() {
    ...
    let n: usize = input.trim().parse().expect("Please provide a valid integer");

    // Compute the series approximation
    let mut sum = 1.0_f64; // corresponds to the initial term for i = 0
    for i in 1..=n {
        // (-1).pow(i) alternates sign; cast to f64 for the division
        let sign = (-1_i32).pow(i as u32) as f64;
        let denominator = (2 * i + 1) as f64;
        sum += sign / denominator;
    }

    // Multiply by 4 and round to 5 decimal places
    let pi_approx = (4.0 * sum * 1e5).round() / 1e5;
