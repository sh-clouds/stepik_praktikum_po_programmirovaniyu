...
fn main() {
    // Read integers from standard input until a line containing only '0' is encountered.
    let stdin = io::stdin();
    ...
    for line_result in stdin.lock().lines() {
        ...
        match trimmed.parse::<i32>() {
            ...
            Err(_) => continue, // Skip lines that cannot be parsed as integers.
        }
    }

    // Count "peak" elements: elements greater than both neighbours.
    let peak_count = if numbers.len() < 3 {
        0
    } else {
        (1..numbers.len() - 1)
            .filter(|&i| numbers[i - 1] < numbers[i] && numbers[i] > numbers[i + 1])
            .count()
    };
