...
fn main() {
    ...
    let mut max_all: i32 = 0;
    let mut max_up: i32 = 0;
    let mut max_down: i32 = 0;

    // Process each integer until a sentinel value `0` is encountered
    for token in input.split_whitespace() {
        let current: i32 = token.parse().unwrap();

        // Stop processing when the sentinel is read
        if current == 0 {
            break;
        }

        // Update upward and downward streak counters
        ...
        // Keep the overall maximum of all three counters
        max_all = max_all.max(max_up).max(max_down);

        // Reset counters when the trend breaks
        max_up = if current > previous_value { max_up } else { 1 };
        max_down = if current < previous_value { max_down } else { 1 };
