...
fn convert(num_str: &str, from_base: u32, to_base: u32) -> String {
    // Try to parse the input number in the source base.
    // If it fails, return "-1" as the Python version does.
    let mut value: i128 = match i128::from_str_radix(num_str, from_base) {
        Ok(v) => v,
        Err(_) => return "-1".to_string(),
    };

    // Zero is a special case – the loop below would produce an empty string.
    if value == 0 {
        return "0".to_string();
    }

    // Build the result digits in reverse order.
    let mut result_rev = Vec::new();
    let to_base_i128 = to_base as i128;
    while value > 0 {
        let remainder = (value % to_base_i128) as usize;
        result_rev.push(DIGITS[remainder] as char);
        value /= to_base_i128;
    }

    // Reverse to obtain the correct order.
    result_rev.iter().rev().collect()
}

fn main() {
    // Read the whole stdin, split by whitespace and collect the three tokens.
    let mut input = String::new();
    io::stdin().read_to_string(&mut input).expect("Failed to read stdin");
    let parts: Vec<&str> = input.split_whitespace().collect();

    if parts.len() != 3 {
        eprintln!("Expected exactly three tokens: <number> <from_base> <to_base>");
        return;
    }

    let n = parts[0];
    let s1: u32 = parts[1].parse().expect("Invalid source base");
    let s2: u32 = parts[2].parse().expect("Invalid target base");

    let output = convert(n, s1, s2);
    println!("{}", output);
}
