...
/// Returns a vector containing the prime factors of `n` (including repetitions),
...
fn main() {
    ...
    // Count occurrences of each distinct prime factor, keeping them sorted.
    let mut count_map: BTreeMap<u64, usize> = BTreeMap::new();
    for p in factors {
        *count_map.entry(p).or_insert(0) += 1;
    }

    // ----- output in the required format -----
    let total = count_map.len();
    for (idx, (prime, exp)) in count_map.iter().enumerate() {
        if *exp == 1 {
            print!("{}", prime);
        } else {
            print!("{}^{}", prime, exp);
        ...
        // Append '*' between factors, but not after the last one.
        if idx + 1 < total {
            print!("*");
        }
    }
