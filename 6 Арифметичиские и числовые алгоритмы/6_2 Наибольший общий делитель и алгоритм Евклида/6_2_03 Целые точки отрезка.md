...
fn main() {
    ...
    let mut x1 = iter.next().expect("missing x1");
    let mut y1 = iter.next().expect("missing y1");
    let mut x2 = iter.next().expect("missing x2");
    let mut y2 = iter.next().expect("missing y2");

    // If the line is vertical, the answer is simple
    if x1 == x2 {
        let result = (y2 - y1).abs() + 1;
        println!("{}", result);
        process::exit(0);
    }

    // Ensure x1 < x2 for the forward iteration
    if x2 < x1 {
        std::mem::swap(&mut x1, &mut x2);
        std::mem::swap(&mut y1, &mut y2);
    }

    // Slope as floating point
    let k: f64 = (y2 - y1) as f64 / (x2 - x1) as f64;

    // Count integer points on the segment, starting with the left endpoint
    let mut count: i64 = 1;

    for x in x1..x2 {
        let y = y1 as f64 + k * ((x - x1) as f64);
        if y.fract() == 0.0 {
            count += 1;
        }
    }
