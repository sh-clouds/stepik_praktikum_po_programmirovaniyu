...
fn main() {
    ...
    let mut first = numbers.next().expect("Missing first integer");
    let mut second = numbers.next().expect("Missing second integer");

    // Ensure `first` is the larger (or equal) number
    if first < second {
        std::mem::swap(&mut first, &mut second);
    }

    // Compute the required value: gcd(first‑1, second‑1) + 1
    let result = gcd(first - 1, second - 1) + 1;
