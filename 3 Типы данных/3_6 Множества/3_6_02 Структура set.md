...
fn main() {
    ...
    let t: usize = lines
        .next()
        .expect("Missing number of commands")
        .trim()
        .parse()
        .expect("Invalid number format");

    // Set to store the integers
    let mut set: HashSet<i32> = HashSet::new();

    // Buffer for all output to print at once
    ...
    for _ in 0..t {
        ...
        if let Some(rest) = line.strip_prefix("ADD") {
            ...
            let number: i32 = rest
                .trim()
                .parse()
                .expect("Invalid number for ADD");
            set.insert(number);
        } else if let Some(rest) = line.strip_prefix("PRESENT") {
            // Command: PRESENT <number>
            let number: i32 = rest
                .trim()
                .parse()
                .expect("Invalid number for PRESENT");
