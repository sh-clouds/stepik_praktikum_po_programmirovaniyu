...
fn main() {
    let n = get_n();
    let mut students = (0..n)
        .map(|_| Student::read_from_stdin())
        .collect::<Vec<Student>>();
    students.sort_by(Student::sorting_function);
    print!(
        "{}",
        students.into_iter()
...
impl Student {
    fn read_from_stdin() -> Student {
        let mut parts = stdin().lines().take(4)
            .map(|s| s.unwrap().trim().to_string());
        return Student {
            surname: parts.next().unwrap(),
            name: parts.next().unwrap(),
            class: Class::from(parts.next().unwrap()),
            date_of_birth: parts.next().unwrap(),
        };
    }

    fn sorting_function(student1: &Self, student2: &Self) -> Ordering {
        let class_key =
            if student1.class.number > student2.class.number {
                Greater
            } else if student1.class.number < student2.class.number {
                Less
            } else if student1.class.letter > student2.class.letter {
                Greater
            } else if student1.class.letter < student2.class.letter {
                Less
            } else {
        ...
        return student1.surname.cmp(&student2.surname);
    }
...
impl Class {
    fn from(string: String) -> Self {
        ...
        return Class {
            number: if chars.len() == 3 {
                format!("{}{}", chars[0], chars[1]).parse::<u8>().unwrap()
            } else {
