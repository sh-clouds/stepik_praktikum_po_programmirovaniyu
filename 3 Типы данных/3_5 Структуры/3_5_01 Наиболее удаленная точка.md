...
fn main() {
    print!(
        "{}", get_points().into_iter().fold(
            Point { x: 0, y: 0 },
            |closest_point, point|
                closest_point.more_distant_from_origin(point)
        ).to_string()
    )
}
...
fn get_points() -> Vec<Point> {
    ...
    return stdin.lines().take(input.trim().parse::<usize>().unwrap())
        .map(|string| Point::from(string.unwrap()))
        .collect();
}
...
impl Point {
    fn more_distant_from_origin(self, other: Point) -> Self {
        if self.distance_to_origin() > other.distance_to_origin() {
            self
        } else {
            other
        }
