...
fn main() {
    ...
    // Remove trailing newline characters (behaviour similar to Python's input())
    ...
    for ch in s.chars() {
        if ch.is_ascii_digit() {
            ...
            // End of a digit block: convert to binary and append the current non‑digit character
            if let Ok(value) = number_buf.parse::<u128>() {
                result.push_str(&format!("{:b}", value));
            } else {
                // Fallback for numbers that don't fit into u128 (unlikely in typical tests)
                result.push_str("0");
            }
            result.push(ch);
            number_buf.clear();
        } else {
            // Normal non‑digit character
            result.push(ch);
        }
    }

    // If the string ends with digits, convert the remaining number
