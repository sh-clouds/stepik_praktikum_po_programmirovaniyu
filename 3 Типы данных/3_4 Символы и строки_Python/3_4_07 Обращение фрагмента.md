...
fn main() {
    ...
    let original = input.trim_end_matches(&['\n', '\r'][..]);

    // Work with a lower‑cased clone only for the search of 'h'
    let lowered = original.to_lowercase();

    // Find the first and last occurrence of 'h' (case‑insensitive)
    let first_h = lowered.find('h');
    let last_h = lowered.rfind('h');

    // If there is no 'h' at all, just output the original string
    let result = match (first_h, last_h) {
        (Some(first_idx), Some(last_idx)) => {
            // Byte indices are safe here because we are dealing with ASCII letters.
            // Prefix up to (and including) the first 'h'
            let prefix = &original[..first_idx + 1];

            // Substring strictly between the two 'h's
            let middle = &original[first_idx + 1..last_idx];
            // Reverse the characters of the middle part
            let reversed_middle: String = middle.chars().rev().collect();

            // Suffix starting from the last 'h'
            let suffix = &original[last_idx..];

            // Assemble the final string
            format!("{}{}{}", prefix, reversed_middle, suffix)
        }
        // No occurrence of 'h' → unchanged
