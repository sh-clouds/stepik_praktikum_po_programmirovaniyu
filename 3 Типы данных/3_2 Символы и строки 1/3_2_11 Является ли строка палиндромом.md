...
fn main() {
    ...
    let sanitized: String = raw_input
        .chars()
        .filter(|c| !c.is_whitespace())
        .collect();

    // Determine if the sanitized string is a palindrome
    let is_palindrome = sanitized
        .chars()
        .eq(sanitized.chars().rev());

    // Output the result
    let result = if is_palindrome { "yes" } else { "no" };
    println!("{}", result);
}
