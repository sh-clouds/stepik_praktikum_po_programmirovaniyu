...
fn main() {
    ...
    let input_string = raw_input.trim_end_matches(&['\n', '\r'][..]);
    ...
    for start_idx in 0..length {
        for end_idx in (start_idx + 1)..=length {
            // Build the substring from start_idx (inclusive) to end_idx (exclusive)
            let candidate: String = characters[start_idx..end_idx].iter().collect();

            if is_palindrome(&candidate) && candidate.len() > longest_palindrome.len() {
