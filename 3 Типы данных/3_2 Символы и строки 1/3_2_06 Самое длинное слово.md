...
fn main() {
    ...
    let mut longest_word: Option<(&str, usize)> = None;
    ...
    for word in input.split_whitespace() {
        ...
        match longest_word {
            // Replace if we find a longer word
            Some((_, current_len)) if length > current_len => {
                longest_word = Some((word, length));
            }
            // First word encountered
            None => {
                longest_word = Some((word, length));
            }
            // Keep the existing longest word
            _ => {}
        }
    }

    // Output the result:
    // first the word, then its length, each on a new line.
    if let Some((word, length)) = longest_word {
        println!("{}\n{}", word, length);
    }
}
