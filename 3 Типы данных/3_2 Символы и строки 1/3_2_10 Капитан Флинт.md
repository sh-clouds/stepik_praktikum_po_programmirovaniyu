...
fn main() {
    // Read the whole stdin into a single string
    ...
    for line in input.lines() {
        // Split the line into command and magnitude
        let mut parts = line.split_whitespace();
        let command = match parts.next() {
            Some(c) => c,
            None => continue, // skip empty lines
        };
        let magnitude_str = match parts.next() {
            Some(m) => m,
            None => continue, // malformed line, ignore
        };
        // Parse the magnitude as an integer
        let magnitude: i32 = magnitude_str
            .parse()
            .unwrap_or_else(|_| panic!("Invalid number: {}", magnitude_str));

        // Update coordinates based on the command
        match command {
            "North" => y += magnitude,
            "South" => y -= magnitude,
            "East"  => x += magnitude,
