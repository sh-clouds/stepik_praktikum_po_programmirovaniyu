...
fn main() {
    ...
    // Parse n and m
    let n: usize = iter.next().expect("missing n").parse().unwrap();
    let m: usize = iter.next().expect("missing m").parse().unwrap();

    // Read the original n × m matrix (as strings)
    let mut matrix: Vec<Vec<String>> = Vec::with_capacity(n);
    for _ in 0..n {
        let mut row: Vec<String> = Vec::with_capacity(m);
        for _ in 0..m {
            let token = iter
                .next()
                .expect("not enough matrix elements")
                .to_string();
            row.push(token);
        }
        matrix.push(row);
    ...
    // new[i][j] = original[n‑1‑j][i]  (transpose with reversed rows)
    let mut transformed: Vec<Vec<String>> = vec![Vec::with_capacity(n); m];
    for i in 0..m {
        for j in 0..n {
            let value = matrix[n - 1 - j][i].clone();
            transformed[i].push(value);
        }
    }

    // Print the result, space‑separated rows
