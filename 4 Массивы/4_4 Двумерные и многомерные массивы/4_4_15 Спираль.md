...
/// Generate the sequence of slice lengths used by the spiral algorithm.
fn slice_lengths(mut n: usize) -> Vec<usize> {
    // The first length is (n - 1)
    n = n.saturating_sub(1);
    let mut lengths = vec![n];

    // Repeatedly append the current length twice, then decrease it by 2,
    // until the length would become zero.
    while n / 2 > 0 {
        lengths.push(n);
        lengths.push(n);
...
/// Produce the coordinates (row, col) of the cells visited by a
/// clockwise outward spiral on an `n × n` grid, starting from the top‑left corner (0,0).
fn iter_spiral(n: usize) -> Vec<(usize, usize)> {
    // Direction vectors for rows (i) and columns (j)
    // i_cycle:  [0,  1, 0, -1]  → down, right, up, left (in terms of row index)
    // j_cycle:  [1,  0, -1, 0]  → right, down, left, up (in terms of column index)
    let i_cycle = [0_i32, 1, 0, -1];
    let j_cycle = [1_i32, 0, -1, 0];

    let slice_lens = slice_lengths(n);
    ...
    for (step, &len) in slice_lens.iter().enumerate() {
        let d_i = i_cycle[step % 4];
        let d_j = j_cycle[step % 4];
