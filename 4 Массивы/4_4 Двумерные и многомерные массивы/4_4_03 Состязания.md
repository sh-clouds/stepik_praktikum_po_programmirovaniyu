...
fn main() {
    let stdin = stdin();
    let n = get_n(&stdin);
    let (index, sum) = stdin.lines().take(n)
        .map(
            |string| {
                let string = string.unwrap();
                string.split_whitespace().fold(
                    0_u32,
                    |sum, result_str|
                    sum + result_str.parse::<u32>().unwrap()
                )
            }
        )
        .enumerate()
...
fn get_n(stdin: &Stdin) -> usize {
    ...
    return buffer.split_whitespace().next().unwrap()
        .parse::<usize>().unwrap();
}
