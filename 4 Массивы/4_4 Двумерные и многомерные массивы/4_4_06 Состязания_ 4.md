...
fn main() {
    ...
    // Split by whitespace and parse into a vector of i32
    let mut iter = input.split_whitespace().map(|s| s.parse::<i32>().unwrap());

    // First two numbers are the dimensions of the matrix: rows (n) and columns (m)
    let rows: usize = iter.next().unwrap() as usize;
    let cols: usize = iter.next().unwrap() as usize;

    // Build the matrix: a vector of rows, each row is a vector of integers
    ...
    // ----- Core logic -----
    // Find the maximum element in the whole matrix
    ...
    // Count how many rows contain this maximum element
    let rows_with_max = matrix
        .iter()
        .filter(|row| row.iter().any(|&x| x == max_value))
        .count();

    // ----- Output -----
    println!("{}", rows_with_max);
}
