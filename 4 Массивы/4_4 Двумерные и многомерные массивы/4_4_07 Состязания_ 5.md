...
fn main() {
    ...
    // Find the overall maximum value `win`
    let win = matrix
        .iter()
        .flat_map(|row| row.iter())
        .copied()
        .max()
        .expect("Matrix must contain at least one element");

    // Collect indices of rows that contain `win`
    let winner_indices: Vec<usize> = matrix
        .iter()
        .enumerate()
        .filter_map(|(idx, row)| {
            if row.contains(&win) {
    ...
    // Output the results
    println!("{}", winner_indices.len());
    if !winner_indices.is_empty() {
        // Print indices separated by a space
        let output = winner_indices
            .iter()
        ...
        // If no rows contain the maximum (shouldn't happen), print an empty line
        println!();
    }
}
