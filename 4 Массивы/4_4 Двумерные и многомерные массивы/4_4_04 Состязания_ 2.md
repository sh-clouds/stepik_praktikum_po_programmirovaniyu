...
fn main() {
    let stdin = stdin();
    let (n, m) = get_n_and_m(&stdin);
    let matrix = stdin.lines().take(n).map(
        |string| string.map(
            |string| string.split_whitespace()
    ...
    let mut maximal_value = &0_u16;
    for i in 0..n {
        for j in 0..m {
            if &matrix[i][j] > maximal_value {
                maximal_value = &matrix[i][j];
                line = i;
                column = j;
            }
        }
...
fn get_n_and_m(stdin: &Stdin) -> (usize, usize) {
