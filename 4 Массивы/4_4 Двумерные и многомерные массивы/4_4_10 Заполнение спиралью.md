...
fn rotate_ccw(matrix: &Vec<Vec<i32>>) -> Vec<Vec<i32>> {
    let rows = matrix.len();
...
fn main() {
    ...
    let k: i32 = input.trim().parse().expect("input must be an integer");

    // ----- matrix size (always square) -----
    let size: usize = (2 * k + 1) as usize;
    let rows = size;
    let cols = size;

    // ----- initialise matrix with -1 -----
    let mut matrix = vec![vec![-1_i32; cols]; rows];

    // ----- iterator that yields decreasing numbers -----
    let mut current: i32 = (rows * cols) as i32 - 1;

    // ----- fill the matrix in a spiral (top → right → bottom → left) -----
    let layers = (std::cmp::min(rows, cols) + 1) / 2; // ceil(min/2)

    for layer in 0..layers {
        // top row (left → right)
        for c in layer..(cols - layer) {
            if matrix[layer][c] == -1 {
                matrix[layer][c] = current;
                current -= 1;
            }
        }

        // right column (top → bottom)
        for r in (layer + 1)..(rows - layer) {
            if matrix[r][cols - layer - 1] == -1 {
                matrix[r][cols - layer - 1] = current;
                current -= 1;
            }
        }

        // bottom row (right → left) – avoid double‑writing the same row
        if rows - layer - 1 != layer {
            for c in (layer..(cols - layer)).rev() {
                if matrix[rows - layer - 1][c] == -1 {
                    matrix[rows - layer - 1][c] = current;
                    current -= 1;
                }
            }
        }

        // left column (bottom → top) – avoid double‑writing the same column
        if cols - layer - 1 != layer {
            for r in ((layer + 1)..(rows - layer - 1)).rev() {
                if matrix[r][layer] == -1 {
                    matrix[r][layer] = current;
                    current -= 1;
                }
            }
        }
    ...
    // ----- rotate the filled matrix three times (equivalent to 270° CCW) -----
