...
fn main() {
    ...
    print!(
        ...
        stdin.lines().take(n).map(
            ...
            (0_usize, vec![0_u16]),
            |last_maximum, current| {
                let current_maximum = current.1.iter().max().unwrap();
                let last_max = last_maximum.1.iter().max().unwrap();
                return if current_maximum > last_max
                    || current_maximum == last_max
                    && last_maximum.1.iter().sum::<u16>() < current.1
                    .iter().sum::<u16>() {
                    current
                } else {
                    last_maximum
