...
/// Count adjacent mines around cell (x, y) in the grid `grid`.
fn count_adjacent(grid: &Vec<Vec<i32>>, x: usize, y: usize, n: usize, m: usize) -> i32 {
    let mut count = 0;
...
fn main() {
    ...
    // ----- initialise grid (0 = empty) -----
    let mut grid: Vec<Vec<i32>> = vec![vec![0; m]; n];

    // ----- mines -----
    let k: usize = it.next().unwrap().parse().unwrap();
    for _ in 0..k {
        let x: usize = it.next().unwrap().parse::<usize>().unwrap() - 1; // 0‑based
    ...
    for i in 0..n {
        for j in 0..m {
            ...
            grid[i][j] = count_adjacent(&grid, i, j, n, m);
        }
    }
