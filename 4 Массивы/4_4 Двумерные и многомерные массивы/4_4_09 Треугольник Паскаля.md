...
fn main() {
    ...
    let n: usize = iter.next().unwrap().parse().expect("expected integer for n");
    let m: usize = iter.next().unwrap().parse().expect("expected integer for m");

    // ----- Build matrix with initial 1s -----
    let mut matrix: Vec<Vec<u64>> = vec![vec![1; m]; n];

    // ----- Fill matrix according to the rule -----
    for i in 1..n {
        for j in 1..m {
