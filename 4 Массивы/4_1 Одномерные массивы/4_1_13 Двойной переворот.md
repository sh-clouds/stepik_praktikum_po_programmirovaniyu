...
fn main() {
    let line = stdin().lines().take(1)
        .next().unwrap().unwrap();
    let mut input = line.split_whitespace();
    let mut sequence =
        (1..=input.next().unwrap().parse::<u16>().unwrap())
            .collect::<Vec<u16>>();
    for _ in 0..2 {
        sequence.reverse_part(
...
trait ReversePart {
    fn reverse_part(&mut self, start: usize, end: usize);
}

impl ReversePart for Vec<u16> {
    fn reverse_part(&mut self, start: usize, end: usize) {
