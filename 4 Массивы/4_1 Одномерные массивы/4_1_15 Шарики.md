...
fn main() {
    ...
    // The first number (n) is not used further, but we pop it to keep the same logic
    if !numbers.is_empty() {
        numbers.remove(0);
    }

    let mut balls = numbers;          // the list of balls
    let mut all_counter: i32 = 0;     // total removed balls
    let mut flag = true;              // continue searching while true

    while flag {
        let mut found_run: Option<(usize, usize)> = None; // (start_index, length)

        // Search for the first run of at least three equal consecutive balls
        ...
        match found_run {
            Some((start, length)) => {
                all_counter += length as i32;
                // Remove the identified run
                balls.drain(start..start + length);
                if balls.is_empty() {
                    flag = false;
                }
            }
            None => {
                // No more runs of length >= 3
                flag = false;
            }
        }
    }
