...
fn rotate_right(arr: &mut [i32], shift: usize) {
    // Cyclic shift to the right by `shift` positions
    arr.rotate_right(shift);
}

fn rotate_left(arr: &mut [i32], shift: usize) {
    // Cyclic shift to the left by `shift` positions
...
fn main() {
    ...
    // List of integers
    let mut lst: Vec<i32> = (0..n)
        .map(|_| iter.next().unwrap().parse().unwrap())
        .collect();

    // k – shift amount, reduced modulo n
    let raw_k: i64 = iter.next().unwrap().parse().unwrap();
    let k = (raw_k % n as i64) as i64;

    // Apply rotation according to the sign of k
    if k > 0 {
        rotate_right(&mut lst, k as usize);
    } else if k < 0 {
        // Convert to positive shift for left rotation
        rotate_left(&mut lst, (-k) as usize);
    }
