...
fn main() {
    let mut lines = stdin().lines().take(2);
    let numbers = lines.next().unwrap().unwrap()
        .split_whitespace()
        .map(|str| str.parse::<i32>().unwrap())
        .collect::<Vec<i32>>();
    let mut numbers_references = numbers.iter().collect::<Vec<&i32>>();
    let k = lines.next().unwrap().unwrap().parse::<usize>().unwrap();
    for i in k..numbers_references.len() - 1 {
        numbers_references[i] = numbers_references[i + 1];
    }
    numbers_references.pop();
    print!(
        "{}",
        numbers_references.iter().enumerate().fold(
            String::new(),
            |mut output, (index, number)| {
                output.push_str(number.to_string().as_str());
                if index != numbers_references.len() - 1 {
                    output.push(' ');
                }
                return output;
            }
