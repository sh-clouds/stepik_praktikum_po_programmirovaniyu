Сжатие списка

Дан список целых чисел. Требуется “сжать” его, переместив все ненулевые элементы в левую часть списка, не меняя их порядок, а все нули - в правую часть. Порядок ненулевых элементов изменять нельзя, дополнительный список использовать нельзя, задачу нужно выполнить за один проход по списку. Распечатайте полученный список.

Входные данные
Вводится список чисел. Все числа списка находятся на одной строке.

Выходные данные
Выведите ответ на задачу.

Sample Input:
4 0 5 0 3 0 0 5

Sample Output:
4 5 3 5 0 0 0 0

Напишите программу. Тестируется через stdin → stdout
Верно решили 355 учащихся
Из всех попыток 73% верных


# ruby
a = gets.chomp.split().map{|j| j.to_i}
#a = [0, 0, 0, 0, 0, 0, 0, 5]
c = a.filter{|i| i === 0}.size
a.delete(0)
c.times do |i| 
    a.push(0)
end   
print a.join(" ")


# python
lst = input().split()
n = lst.count('0')
counter, step = 0, 0

while counter <= n:
    if lst[step] == '0':
        del lst[step]
        lst.append('0')
        counter += 1
    else:
        step += 1
print(*lst)


# JS
var stdin = process.openStdin();

stdin.addListener("data", function(d) {
    let str = d.toString().split(' ');
    let index = 0;
    for (let i=0; i<str.length; i++) 
        if (str[i] !== '0') {
            if (index !== i) {
            str[index] = str[i];
            str[i] = 0;
            }
            index++;        
        }     
    let result = '';
    for (let key of str) result = result+key+' ';
    console.log(result);
  });


# C#
using System;
using System.Linq;

public class MainClass
{
    public static void Main()
    {
        var arr=Console.ReadLine().Split().ToList();
        int originalLength=arr.Count;
        arr.RemoveAll(s=>s=="0");
        string item;
        for(int i=0;i<originalLength;i++)
        {
			Console.Write((item=i<arr.Count?arr[i]:"0")+ " ");
        }
    }
}


# java
import java.util.Scanner;
class Main {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        int num = 0;
        int count = 0;
        while(sc.hasNext()){
            num = sc.nextInt();
            if (num != 0) { System.out.printf("%d ", num); }
            else          { count++; }
        }
        sc.close();
        System.out.print("0 ".repeat(count));
    }
}



# C
#include <inttypes.h>
#include <stdio.h>
#include <stdlib.h>

void input(int64_t** arr, size_t* N);
void output(int64_t** arr, size_t N);

int main() {
    size_t N = 0, K = 0;
    int64_t* array;
    int64_t X;
    input(&array, &N);

    size_t current = 0;
    for (size_t i = 0; i < N; i++) {
        if (array[i] == 0) continue;
        if (i > current) {
            array[current] = array[i];
            array[i] = 0;
        }
        current++;
    }

    output(&array, N);
    if (N) free(array);
    return 0;
}

void output(int64_t** arr, size_t N) {
    for (size_t i = 0; i < N; i++) {
        printf("%"PRId64, (*arr)[i]);
        if (i != N - 1) printf(" ");
    }
}

void input(int64_t** arr, size_t* N) {
    int64_t num;
    while (scanf("%"SCNd64, &num) == 1) {
        (*N)++;
        if ((*N) == 1) (*arr) = malloc(sizeof(int64_t));
        else (*arr) = realloc((*arr), (*N) * sizeof(int64_t));
        (*arr)[(*N) - 1] = num;
    }
}


# C++
#include <iostream>
#include <vector>
#include <algorithm>

int main() {
    // put your code here
    std::vector  <int> arr;
    
    int temp;
    while(std::cin >> temp)
    {
        arr.push_back(temp);
    }
     
    std::stable_partition(arr.begin(), arr.end(), [](int n){return n != 0;});
    
    for(auto &&ptr : arr)
    {
        std::cout << ptr << " ";
    }
    return 0;
}

# rust
use std::io::stdin;

fn main() {
    let numbers = stdin().lines().next().unwrap().unwrap()
        .split_whitespace()
        .map(|str| str.parse::<i32>().unwrap())
        .collect::<Vec<i32>>();
    let mut numbers_references = numbers.iter().collect::<Vec<&i32>>();
    let mut vacant_position = 0_usize;
    for i in 0..numbers_references.len() {
        if numbers_references[i] != &0 {
            let temp = numbers_references[vacant_position];
            numbers_references[vacant_position] = numbers_references[i];
            numbers_references[i] = temp; 
            vacant_position += 1;
        }
    }
    print!(
        "{}",
        numbers_references.iter().enumerate().fold(
            String::new(),
            |mut output, (index, number)| {
                output.push_str(number.to_string().as_str());
                if index != numbers_references.len() - 1 {
                    output.push(' ');
                }
                return output;
            }
        )
    )
}